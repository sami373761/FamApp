...
Expo SDK 57 · React Native 0.86 · React 19.2 · expo-router 7 (file-based) · TypeScript 6 strict · React Compiler on.
...
past `visible` — a `mounted` state set during render and cleared by the exit's completion,
the two halves being asymmetric on purpose: the Modal has to exist in the frame the entrance
starts, while only the animation knows when it may go — because unmounting on the prop cuts
the exit off at its first frame. Three consequences worth knowing: the backdrop is
...
goes through one `apply` so the two cannot drift. It is read as `state.current.x` rather than
unwrapped once at the top, since every reader is a handler and React Compiler refuses a `.current`
touched during render. And `onClose` is read **through a ref**: the responder is memoised for the
component's life while the prop is a fresh arrow on every render of the bubble, so calling it
directly would pin the first render's closure forever; the ref is refreshed in an effect, for the
same compiler reason. **Pinch does not
...
- **`StyleSheet.absoluteFill` is a plain object in RN 0.86, and `absoluteFillObject` is gone.** It is removed from both the typings and the runtime, so the SDK 54 form (`...StyleSheet.absoluteFillObject`) now fails to typecheck. Spread `absoluteFill` directly where the four insets are being merged into a larger style, and pass it bare where a whole `style` is expected (`family-map.tsx`) — one name covers both jobs now. RN 0.81 had this exactly the other way round, so a snippet written against SDK 54 will be backwards.
...
- The template's `NativeTabs` (`expo-router/unstable-native-tabs`) was **deliberately replaced** with expo-router's JS `Tabs` so all five tabs theme identically across iOS and Android. Do not switch back. In expo-router 7 that import is **`expo-router/js-tabs`**; the `Tabs` still exported from `expo-router` itself is deprecated.
...
- **`eslint-config-expo@57` ships eslint-plugin-react-hooks 6.x, and its React Compiler rules are errors.** `react-hooks/refs`, `set-state-in-effect`, `purity` and `preserve-manual-memoization` did not exist under the 5.x that SDK 54 shipped, so code that was clean then is not automatically clean now. React Compiler has been **on** the whole time (`app.json` → `experiments.reactCompiler`); what changed at SDK 57 is that its bail-outs are reported rather than silent. Four consequences worth knowing before writing anything animated:

  - **`useRef(new Animated.Value(0)).current` is refused**, and the replacement is lazy state: `const [progress] = useState(() => new Animated.Value(0))`. Identical semantics — constructed once, never reassigned — and it is what every animated component in the app now uses. Do **not** reach for `useMemo` here: the compiler is free to drop a memo, and a re-run would hand the component a fresh `Animated.Value` mid-animation.
  - **A ref may not be written during render.** The prop-mirror pattern (`closeRef.current = onClose`, so a memoised `PanResponder` reads the latest prop instead of the first render's closure) moves into an effect keyed on the prop. Every reader is a gesture or a timer, neither of which can fire before the commit that effect belongs to.
  - **`setState` at the top of an effect to reset state when a prop changes** becomes a render-phase adjustment (`if (path !== signedPath) { … }`), which is React's own documented replacement and is strictly better here: it also removes the frame in which the old value is still on screen under the new prop.
  - **A `useCallback` whose dependency list narrows an optional (`profile?.id`) and then reads it back off the parent (`profile.id`) makes the compiler infer `profile`**, which does not match and skips optimisation for the whole component. Read it into a local first.

  Four sites are genuinely irreconcilable and carry a scoped `eslint-disable-next-line` with its reason, rather than being contorted: the two `PanResponder.create` calls (`sheet.tsx`, `image-viewer.tsx`) — the rule reads every `.current` inside a `useMemo` body as a render-time access, and a `PanResponder`'s handlers run when a finger moves; `image-viewer.tsx`'s reset, which has to sit beside an `Animated` call and so cannot leave the effect; `date-field.tsx`'s `Date.now()`, because "is this date in the future" has no answer that is not read off the clock. Prefer restructuring; disable only where the rule and a documented design genuinely disagree.

- `src/hooks/use-color-scheme.web.ts` sets state in an effect on purpose — that is the hydration-detection pattern, not a mistake. It now carries `// eslint-disable-next-line react-hooks/set-state-in-effect`. Under `eslint-config-expo@10` it deliberately carried none, because plugin 5.x had no such rule and naming an unknown rule in a disable comment is itself a lint **error** ("Definition for rule ... was not found"); the 6.x that SDK 57 brings is what made the comment legal and necessary.

- **`eslint.config.js` ignores `supabase/functions`, for the reason `tsconfig.json` excludes it.** Edge Functions are Deno: their `jsr:` and `https:` specifiers are resolvable by Deno alone, so the app's resolver reports every one as `import/no-unresolved`.
